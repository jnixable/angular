...
class WithdrawEndpointIT extends BaseIT {
    ...
    void approvedWithdrawDecreasesBalanceAndRecordsTransaction() {
        var response = restTemplate.postForEntity(
                WITHDRAW_URL,
                authorized(Map.of("accountNumber", ALICE_ACCOUNT_NUMBER, "currency", "EUR", "amount", "40.00"), ALICE_CODE),
        ...
        assertThat(balanceOf(ALICE_ACCOUNT_NUMBER, Currency.EUR)).isEqualByComparingTo("60.00");
    ...
    void rejectedWithdrawReturnsUnprocessableEntityAndKeepsBalance() {
        ...
        var response = restTemplate.postForEntity(
                WITHDRAW_URL,
                authorized(Map.of("accountNumber", ALICE_ACCOUNT_NUMBER, "currency", "EUR", "amount", "41.00"), ALICE_CODE),
                String.class);
    ...
    void insufficientFundsReturnsBadRequest() {
        var response = restTemplate.postForEntity(
                WITHDRAW_URL,
                authorized(Map.of("accountNumber", ALICE_ACCOUNT_NUMBER, "currency", "EUR", "amount", "150.00"), ALICE_CODE),
                String.class);
    ...
    void withdrawFromForeignAccountReturnsForbidden() {
        var response = restTemplate.postForEntity(
                WITHDRAW_URL,
                authorized(Map.of("accountNumber", ALICE_ACCOUNT_NUMBER, "currency", "EUR", "amount", "40.00"), BOB_CODE),
                String.class);
    ...
    void missingTokenReturnsUnauthorized() {
        var response = restTemplate.postForEntity(
                WITHDRAW_URL,
                Map.of("accountNumber", ALICE_ACCOUNT_NUMBER, "currency", "EUR", "amount", "40.00"),
                String.class);
